...
# ---------- Orders dashboard: trend, by status, by channel, by region, top products ----------
...
def orders_trend_daily(df):
    ...
    Uses date column and orders column (sum per day) or row count per day.
    """
    df = df.copy()
    date_col = _find_date_col(df)
    if date_col is None or date_col not in df.columns:
    ...
    agg = agg.sort_index()
    # Limit to last 60 points to keep chart readable
    if len(agg) > 60:
        agg = agg.tail(60)
    return {
...
def orders_by_status_component(df):
    ...
    """
    col = _find_column_by_keywords(df, ["status", "order status", "state", "order state", "fulfillment"])
    if col is None:
...
def orders_by_channel_component(df):
    """
    Find a channel/source column and return order counts (or row counts) for horizontal bar chart.
    Returns list of { "name": str, "orders": int, "fill": str }.
    """
    col = _find_column_by_keywords(df, ["channel", "source", "sales channel", "platform", "payment_method", "payment method"])
    if col is None:
        return None
    if "orders" in df.columns:
        df = df.copy()
...
def orders_by_region_component(df):
    ...
    Returns list of { "name": str, "orders": int }.
    """
    geo_col = None
...
def top_products_by_orders_component(df):
    ...
    Returns list of { "product": str, "orders": int, "revenue": float, "avgQty": float }.
    """
    product_col = None
    ...
    # Build rows: product, orders, revenue, avgQty (orders per row = count; avg qty could be quantity column if exists)
    result = []
